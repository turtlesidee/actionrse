...
import { add_strategy_form, add_strategy_form_open, initial_values } from "./logic"
import { FormikProps } from "../../../../../../infrastructure/components/inputs/types"


type AddStrategyProps = {
  formik_parent: FormikProps
}

const AddStrategy: FC<AddStrategyProps> = ({ formik_parent }) => {
  const [open, setOpen] = useRecoilState(add_strategy_form_open)
  const formik = useFormik({
    initialValues: initial_values,
    onSubmit: (values) => {
      formik_parent.setFieldValue('strategies', $add(formik_parent.values.strategies, values))
      setOpen(false)
      formik.resetForm()
    },
    validate: validateSchema(add_strategy_form)
  })
  return (
    <Dialog onClose={() => {}} open={open}>
      <div className="bg-neutral-100 w-[600px] absolute left-1/2 -translate-x-2/4 px-[30px] border border-neutral-400 rounded-xl top-1/2 -translate-y-2/4">
        ...
        <form onSubmit={formik.handleSubmit}>
          <div>
            <InputBox formik={formik} name="name" label="Strategie" className="mb-[18px]"/>
          </div>
